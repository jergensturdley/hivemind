import { HARNESSES, type HarnessId, isHarnessId } from "@/lib/harnesses";
...
export function workerPool(preferred: string): HarnessId[] {
  const ext = HARNESSES.filter((h) => h.id !== HOME_HARNESS).map((h) => h.id);
  const pref = isHarnessId(preferred) && preferred !== HOME_HARNESS ? preferred : null;
  if (!pref) return ext;
  return [pref, ...ext.filter((id) => id !== pref)];
}

/** Atlas routing: implementation fans out; everything else returns home. */
export function routeTasks(titles: string[], preferred: string): HarnessId[] {
  const workers = workerPool(preferred);
  let w = 0;
